/*
 * drm_ioctl_shim.c -- DRM ioctl shim for phoc/phosh on FuriOS Furiphone FLX1/s
 *
...
int drmModeAddFB2WithModifiers(int fd, uint32_t w, uint32_t h, uint32_t fmt,
    ...
    fmap_insert(handles[0], id); return 0;
}
...
int ioctl(int fd, unsigned long request, ...) {
    ...
    if (nr == 0xb8) {
        ...
        if (!frame_w) { frame_w = w; frame_h = h; }
        if (!dumb_map) init_dumb(fd);
        uint32_t id = next_fake++; fb[6] = id;
        fmap_insert(gem, id); ret = 0;
